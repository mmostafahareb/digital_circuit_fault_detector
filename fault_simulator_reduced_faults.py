...
def minimize_faults(gates, single_fault_list):
    minimized_fault_list = []
    output_to_input_faults = {}

    for gate_output, gate_info in gates.items():
        gate_type = gate_info['type']
        gate_inputs = gate_info['inputs']

        equivalent_faults = []
        if gate_type in ['AND', 'NAND']:
            equivalent_faults = [{input_id: False} for input_id in gate_inputs]
        elif gate_type in ['OR', 'NOR']:
            equivalent_faults = [{input_id: True} for input_id in gate_inputs]

        output_fault = {gate_output: gate_type in ['NAND', 'NOR']}
        output_to_input_faults[str(output_fault)] = equivalent_faults

    for fault in single_fault_list:
        is_equivalent = False
        for output_fault_str, equivalent_faults in output_to_input_faults.items():
            if fault in equivalent_faults:
                output_fault = eval(output_fault_str)
                minimized_fault_list.append(output_fault)
                is_equivalent = True
                break
        if not is_equivalent and fault not in minimized_fault_list:
            minimized_fault_list.append(fault)

    minimized_fault_list = [dict(t) for t in {tuple(d.items()) for d in minimized_fault_list}]

    return minimized_fault_list
...
def main():
    ...
    single_fault_list = minimize_faults(gates, get_potential_fault_list(inputs, gates, fanout_map))
    minimized_fault_list = minimize_faults(gates, single_fault_list)
    ...
    detected_faults = []   
    undetectable_faults = []
    for fault in minimized_fault_list:
